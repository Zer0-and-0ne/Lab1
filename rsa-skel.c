...
int rsa_keyGen(size_t keyBits, RSA_KEY* K)
{
	...
	//Using mpz_t/mpz_inits to initialize variables Φ, temp
    mpz_t phi, temp1, temp2;
    mpz_inits(phi, temp1, temp2, NULL);	

	/*using size_t to initialize the number of bytes that is used
	 *as a parrameter to allocate memory using malloc 
	 *for the buf array*/
    size_t primeBytes = keyBits / 16; 
    unsigned char* buf = malloc(primeBytes);

    //Using prf.c/randBytes we generate the prime number p
    randBytes(buf, primeBytes);
    BYTES2Z(temp1, buf, primeBytes);
	if(ISPRIME(temp1) != 2)
		mpz_nextprime(temp1, temp1);
	mpz_set((*K).p, temp1);
	
	//Using prf.c/randBytes we generate the prime number q
	randBytes(buf, primeBytes);
    BYTES2Z(temp2, buf, primeBytes);	
	if(ISPRIME(temp2) != 2)
		mpz_nextprime(temp2, temp2);
	mpz_set((*K).q, temp2);

	//computes n=p*q by using mpz_mul
	mpz_mul((*K).n, (*K).p, (*K).p);

	//generate public e that's coprime of Φ
    mpz_sub_ui(temp1, temp1, 1); // p-1
    mpz_sub_ui(temp2, temp2, 1); // q-1
    mpz_mul(phi, temp1, temp2);  // Φ(n) = (p-1)(q-1)
    mpz_set_ui(temp1, 1);
    do{ mpz_add_ui(temp1, temp1, 1);
    	mpz_gcd(temp2, phi, temp1);
	} while(mpz_cmp_ui(temp2, 1) != 0);		
	mpz_set((*K).e, temp1);


	// compute private exponent d as an inverse of e * mod φ(n)
		...
		RSA_KEY* K)
{
    ...
    size_t outbufSize;
	mpz_t cipherT, plainT;
	mpz_inits(cipherT, plainT, NULL);
    BYTES2Z(plainT, inBuf, len);

	//encrypts the plaintext using public e and n
	mpz_powm(cipherT, plainT, (*K).e, (*K).n);
	
	Z2BYTES(outBuf, outbufSize, cipherT);
	mpz_clears(cipherT, plainT, NULL);
	return outbufSize; /* TODO: return should be # bytes written */
}
		...
		RSA_KEY* K)
{
	...
	//decrypts the ciphertext using public d and n
	mpz_powm(plainT, cipherT, (*K).d, (*K).n);
